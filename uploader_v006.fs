...
\ boolean
0 variable state_compiletoram? \ store state so that it an be restored after initializeDlBuf
...
: save_state_compiletoramOrFlash compiletoram? state_compiletoram? ! ;
: restore_state_compiletoramOrFlash state_compiletoram? @ not if compiletoflash then ;

\ Prompt the user the upload the code file.  Download the file but do not evaluate it.
: ul save_state_compiletoramOrFlash initializeDlBuf  showDlBufSize ." Begin upload: " download CR CR dlBuf_i @ dup dlBuf_numChars ! . ." characters uploaded." CR CR restore_state_compiletoramOrFlash ;
